...
generate_noise_points <- function(n) {
	...
	# sf_points <- st_sample(sf_convex_hull, size = n, type ="random")
	...
	ggplot()+
		...
		geom_sf(data=sf_sampled_points)
	return(sf_sampled_points)
}
...
# Output: ...
# Action: ...
generate_noise_lines <- function(sf_startpoints) {
...
get_avg_radius_bbox <- function(sf_convex_hull) {
	...
	average_radius <- mean(as.numeric(distances))
	return(average_radius)
}



get_degree_furthest_direction <- function(sf_convex_hull, sf_point) {
	hull_vertices <- st_cast(sf_convex_hull, "POINT")
	distances <- st_distance(sf_point, hull_vertices)
	max_distance_index <- which.max(distances)
	max_dist <- max(distances)
	farthest_point <- hull_vertices[max_distance_index, ]
	angle <- atan2(
		st_coordinates(farthest_point)[2] - st_coordinates(sf_point)[2],
		st_coordinates(farthest_point)[1] - st_coordinates(sf_point)[1]
	)
	angle_degrees <- (angle * 180 / pi) %% 360
	return(list("degree" = angle_degrees,
							"distance" = max_dist))
}
...
create_synth_dataset <- function(n_clusters, n_noiseflows, avg_radius) {
	...
	for (i in 1:n_clusters) {
	  ...
	  # cos(base_angle)
	  # variance_length_flows <- runif(1, 20, 50)
	  # variance_angles <- runif(1, 0, 0.7)
...
# Main
################################################################################
avail_networks <- psql1_get_available_networks(con)
avail_networks
...
sf_clusters$origin_geom <- lwgeom::st_startpoint(sf_clusters)
sf_clusters$o_closest_point <- sf_clusters$origin_geom
sf_clusters$dest_geom <- lwgeom::st_endpoint(sf_clusters)
sf_clusters$d_closest_point <- sf_clusters$dest_geom
table(sf_clusters$cluster_id)
