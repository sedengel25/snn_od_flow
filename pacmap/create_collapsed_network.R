...
list.files(path = path_grass, pattern = "collapsed.shp")
filename <- "dd_synth_2po_4pgr_collapsed"
shp_file <- paste0(filename, ".shp")
sf_network <- st_read(here::here(path_grass, shp_file)) %>% as_sfnetwork()
...
sf_network <- sf_network %>%
	...
	mutate(
		id = cat,                           # id entspricht der Spalte 'cat'
	...
	select(id, source, target, m, geometry) # Wähle die gewünschten Spalten aus

# Konvertiere zurück zu einem sf-Objekt
sf_network <- sf_network %>%
	as.data.frame() %>%
	st_as_sf()
